...
export const pluginArchiveType = pgEnum("plugin_archive_type", [
  "single",
  "multi",
]);
export const pluginSourceType = pgEnum("plugin_source_type", [
  "hosted",
  "vcs",
  "local",
  "other",
]);
export const privilege = pgEnum("privilege", [
  "read",
  "write",
  "publish",
  "ultimate",
...
export const plugins = pgTable(
  ...
  (table) => [
    ...
    check(
      "url_source_type_check",
      sql`(source_type <> ALL (ARRAY['vcs'::plugin_source_type, 'hosted'::plugin_source_type])) OR (url IS NOT NULL)`,
    ),
    check(
      "vcs_source_type_check",
      sql`(source_type <> 'vcs'::plugin_source_type) OR (vcs IS NOT NULL)`,
    ),
  ],
);
...
export const authorizationSessions = pgTable(
  ...
  (table) => [
    index("idx_login_sessions_expiry").using(
      "btree",
...
export const packagePublishingTasks = pgTable(
  "package_publishing_tasks",
  {
    id: uuid()
      .default(sql`uuid_generate_v4()`)
      .primaryKey()
...
export const packageContributors = pgTable(
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.packageId, table.contributorId],
      name: "package_contributors_pkey",
    }),
  ],
);
