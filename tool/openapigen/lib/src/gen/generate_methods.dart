...
Iterable<Method> generateMethods(
  ...
  JSArray<OAPIGenMethods> methods, {
      ...
      (m) => m
        ..docs.addAll(
          [
            ...
            if (entriesFromRecord(
              method.returns!,
            ).toDart.where((v) => v.$0.toDart != '200').isNotEmpty)
        ...
        ..optionalParameters.addAll(
          method.parameters.toDart.map((param) {
            var type = 'string';

            if (param.schema.getProperty('type'.toJS).dartify() as String ==
                'string') {
              type = 'string';
            } else if (param.schema.getProperty('type'.toJS).dartify()
                    as String ==
                'number') {
              type = 'number';
            } else if (param.schema.getProperty('type'.toJS).dartify()
