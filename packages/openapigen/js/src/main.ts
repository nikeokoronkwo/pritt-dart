...
function openapiDocToResult(spec: OpenAPIV3.Document): OpenAPIGenResult {
    ...
    function transformBodySchema(responseInfo: OpenAPIV3.ReferenceObject | OpenAPIV3.RequestBodyObject | undefined): {
        type: string,
        name?: string,
        schema?: OpenAPIV3.SchemaObject,
        required?: boolean,
        ...
        if (responseInfo) {
            if ('$ref' in responseInfo) {
                ...
                const [[contentType, obj]] = Object.entries(responseInfo.content as {
                    [media: string]: OpenAPIV3.MediaTypeObject;
                })

                let name = obj.schema?.title ? (obj.schema.title !== 'schema' ? obj.schema?.title : 'unknown') : undefined;
    ...
    function generateMethod(operation: OpenAPIV3.OperationObject | undefined, name: string, path: string, parameters: OpenAPIV3.ParameterObject[] = []) : OAPIGenMethods {
        return {
            method: name,
            path: path,
            ...
            parameters: [...[...(operation?.parameters as OpenAPIV3.ParameterObject[] ?? []), ...(parameters)]],
            body: transformBodySchema(operation?.requestBody),
            returns: Object.assign(
                {},
                ...Object.entries(operation?.responses ?? {}).map(([code, responseInfo]) => {
    ...
    return {
        ...
        methods: Object.entries(spec.paths ?? {}).map(([k, v]) => {
            ...
            if (v?.get) {
                returnArray.push(generateMethod(v.get, 'GET', k, v.parameters as OpenAPIV3.ParameterObject[]));
            }
            if (v?.post) {
                returnArray.push(generateMethod(v.post, 'POST', k, v.parameters as OpenAPIV3.ParameterObject[]));
            }
            if (v?.delete) {
                returnArray.push(generateMethod(v.delete, 'DELETE', k, v.parameters as OpenAPIV3.ParameterObject[]));
            }
            if (v?.put) {
                returnArray.push(generateMethod(v.put, 'PUT', k, v.parameters as OpenAPIV3.ParameterObject[]));
            }
            return returnArray;
