...
final handler = defineRequestHandler((event) async {
  ...
  final pkgVersion = getParams(event, 'version') as String;
  final version = Version.parse(pkgVersion);
  ...
  try {
    ...
    if (body.yank ?? false) {
      ...
      final _ = await crs.db.yankVersionOfPackage(pkgName, version);
    ...
    switch (e.type) {
      case CRSExceptionType.PACKAGE_NOT_FOUND:
        ...
        return common.NotFoundError(
          error: 'The given package ${scopedName(pkgName)} could not be found',
        );
      default:
