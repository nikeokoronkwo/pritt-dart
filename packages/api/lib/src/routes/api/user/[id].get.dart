  ...
  try {
    ...
    final approvedPkgsGroup = StreamGroup<(Package, Iterable<Privileges>)>()
      ..add(
        (root
                ? packages
                : packages.asyncMap((pkg) async {
                    if (pkg.public ?? true) return pkg;

                    if (pkg.scoped) {
                      // get scope members
                      final members = crs.db.getMembersForOrganizationStream(
                        pkg.scope!,
                      );
            ...
            .map((pkg) => (pkg, [Privileges.ultimate])),
      )
      ..add(
        root
            ? contributedPackages
            : contributedPackages.asyncMap((pkg) async {
