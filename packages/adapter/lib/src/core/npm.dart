...
final npmAdapter = Adapter(
  id: 'npm',
  language: 'javascript',
  proxyEndpoints: ['https://registry.npmjs.org/'],
  resolve: (resolve) {
  ...
  request: (req, crs) async {
    ...
    for (final ver in packageVersions) {
      if (versionGroups.containsKey(ver.versionType)) {
        versionGroups[ver.versionType] = versionGroups[ver.versionType] == null
            ? ver
          ...
          return MapEntry(
            ...
            NpmPackage.fromPackageJson(
              ...
              dist: NpmDist(
                ...
                signatures: v.signatures.map(
                  (s) => {'keyid': s.publicKeyId, 'sig': s.signature},
                ),
              ),
