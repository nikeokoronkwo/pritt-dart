...
Future<void> transformTemplates(
  String inputDir,
  String templateDir,
  String outputDir,
  ...
  for (final codeMap in configurationCode) {
    ...
    (await File(outPath).create(recursive: true)).writeAsStringSync(codeMap.code);
  }
  ...
  for (final file in files) {
    final destPath = path.join(outputDir, path.relative(templateDir, file.parentPath), file.name);
