...
function assertHelpDescription(text: string, global: boolean = false) {
  ...
  assert(lines.find(l => l.includes('Usage:')) !== undefined, "Should contain usage description");
  assert(lines.find(l => l.includes('-h, --help')) !== undefined, "Should contain help option");

  if (global) assert(lines.find(l => l.includes('Global options:')) !== undefined, "Global help should have options");
  if (global) assert(lines.find(l => l.includes('Available commands:')) !== undefined, "Global help should have command overview");
}
...
describe('Command-Line E2E Testing', async () => {
  ...
  test("[CALL <empty>,'--help','help']: Equal output", { timeout: 10000 }, () => {
    const baseCall = invokeCLI([]);
