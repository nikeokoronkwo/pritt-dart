...
Future<String?> _runCommand(String executable, List<String> args,
    {ProcessManager? manager, String? directory, Logger? logger}) async {
  logger ??= Logger();
  manager ??= ProcessManager();
  final process = await manager.spawn(executable, args,
...
Future<String?> getVcsRemoteUrl(VCS vcs, {String? directory}) async {
  return switch (vcs) {
    VCS.git => await _runCommand(
        'git', ['config', '--get', 'remote.origin.url'],
        directory: directory),
    VCS.svn => await _runCommand('svn', ['info', '--show-item', 'url'],
        directory: directory),
    VCS.fossil => await _runCommand('fossil', ['remote-url'],
            directory: directory) ??
    ...
    VCS.mercurial =>
      await _runCommand('hg', ['paths', 'default'], directory: directory),
    _ => null
  };
}
...
Future<List<String>> getIgnoredVCSFiles(VCS vcs, {String? directory}) async {
  ...
  switch (vcs) {
    case VCS.git:
      return splitter.convert(await _runCommand(
    ...
    case VCS.fossil:
      return splitter.convert(
          await _runCommand('fossil', ['extras'], directory: directory) ?? '');
    case VCS.svn:
