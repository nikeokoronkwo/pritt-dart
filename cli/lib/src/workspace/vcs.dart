...
Future<String?> getVcsRemoteUrl(VCS vcs, {String? directory}) async {
  return switch (vcs) {
    VCS.git => await _runCommand('git', [
      'config',
      '--get',
      'remote.origin.url',
    ], directory: directory),
    VCS.svn => await _runCommand('svn', [
      'info',
      '--show-item',
      'url',
    ], directory: directory),
    VCS.fossil =>
      await _runCommand('fossil', ['remote-url'], directory: directory) ??
          await _runCommand('fossil', ['info'], directory: directory).then((
            out,
          ) {
            final match = RegExp(r'url:\s*(.*)').firstMatch(out ?? '');
            return match?.group(1);
          }),
    VCS.mercurial => await _runCommand('hg', [
      'paths',
      'default',
    ], directory: directory),
    _ => null,
  ...
  switch (vcs) {
    ...
    case VCS.svn:
      ...
      return splitter
          .convert(result ?? '')
          .where(
            (line) => line.isNotEmpty && line.startsWith('I'),
