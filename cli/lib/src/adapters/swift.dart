...
final swiftHandler = Handler(
  ...
  packageManager: PackageManager(
    ...
    onAdd: () {
      return PackageCmdArgs(
        args: ['swift', 'package' 'add-dependency'],
  ...
  onGetConfig: (directory, controller) async {
    ...
    if (v == null) {
      throw Exception('Could not get version of Swift package. Please specify a version in your pritt.yaml file.');
    }
    ...
    final _ = await controller.run(
      'swift',
      args: ['package-registry', 'login', '--token', r'$PRITT_AUTH_TOKEN', controller.instanceUri],
    );

    // set the registry
    final _ = await controller.run(
      'swift',
      args: ['package-registry', 'set', if (Uri.parse(controller.instanceUri).scheme == 'http') '--allow-insecure-http', controller.instanceUri],
    );
